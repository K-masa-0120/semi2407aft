...
# 混雑度に基づく報酬の計算
def calculate_reward(current_node, next_node, time_id):
    adjacent_nodes, congestion_levels, _ = enode.nodes[current_node]
    if next_node in adjacent_nodes:
        index = adjacent_nodes.index(next_node)
        congestion_level = congestion_levels[index][time_id]
        return -congestion_level  # 混雑度が低いほど報酬が高くなるように負の値にする
    else:
        return -10  # 不正な移動に対するペナルティ
...
for episode in range(num_episodes):
    ...
    while not done and steps < 100:  # 100回移動したら失敗
        ...
        reward = 1 if next_node == goal_node else calculate_reward(int(state[0]), next_node, time_id)
        total_reward += reward
